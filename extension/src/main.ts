...
// Note that this pattern is even
...
// used to log useful but non-sensitive information. In case something goes
// wrong, glancing at the output logs is the first port of call to determine
// what was happening. In VSCode, this output can be seen via the 'Output'
// window, then selecting 'CUE' from the dropdown that allows you to select the
// category of output to view.
//
// TODO: include information about how the instance of the LSP creates a
// separate output channel. And work out if we can tidy up old instances when
// the LSP restarts (e.g. on a config change).
...
// architecture).
//
// Each VSCode window represents a separate workspace or folder. A single
// instance of an extension runs per workspace. If you open multiple VSCode
// windows, each window may host its own instance of the extension.
...
export const output = vscode.window.createOutputChannel('CUE', { log: true });
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
	...
	output.debug('activate debug');
	console.log('vscode-cue activate');
...
export async function deactivate(): Promise<void> {
	console.log('vscode-cue deactivate');
	if (ctx === undefined) {
		// Nothing to do
...
// cmdStartLSP is used to explicitly (re)start the LSP server. It can only be called
// if the extension configuration allows for it.
...
async function extensionConfigurationChange(s: vscode.ConfigurationChangeEvent | undefined): Promise<void> {
	...
	// considering the concrete configuration. This appears to be mitigated by
	// calling s.affectsConfiguration('cue') to determine if there has been a
	// change. We might need to modify this to be a comparison of the net
	// configuration (post applyConfigDefaults) but for now, given the defaults
	// are static, the VSCode check suffices.
	...
	console.log(
		`vscode-cue configuration updated from: ${JSON.stringify(oldConfig, null, 2)}\nto:${JSON.stringify(config, null, 2)}`
	);

	let _, err;
	if (config.useLanguageServer) {
		// TODO: we might want to revisit just blindly restarting the LSP. But for
		// now it's good enough.
		[_, err] = await ve(runCueLsp());
	} else {
		[_, err] = await ve(stopCueLsp());
	}
	if (err !== null) {
...
// it is assumed is safe because the value will generally have come from a call
// to getConfiguration.
