...
export async function activate(context: vscode.ExtensionContext): Promise<undefined> {
  ...
  registerCommand('vscode-cue.welcome', WelcomeCUE);
...
async function installCUEpls(targetPath: string): Promise<Err> {
  ...
  err = await untarCUEpls(stream.Readable.fromWeb(resp!.body!), targetFile);
...
async function untarCUEpls(source: stream.Readable, dst: fs.WriteStream): Promise<Err> {
  ...
  err = await e(stream.promises.pipeline(src, dst));
  if (err !== null) {
...
// ve converts a void promise to a promise that returns an error value, the
// error being the value "caught" in case the input promise is rejected. When
// used with 'await', this allows JavaScript-native Promise-aware functions
// that otherwise encourage the use of try-catch with 'await' to transform
// results into a more Go-style of error handling.
//
// TODO: can we merge this somehow with ve below? Maybe? Might require some
// generics gymnastics.
function e(p: Promise<void>): Promise<Err> {
  return p.then(
    () => {
      return null;
    },
